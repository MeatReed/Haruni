...
class Player {
  ...
  public sendFilters(options: any): void {
    if (options.equalizer) {
      if (options.equalizer.bands > 14 || options.equalizer.bands < 0)
        throw new RangeError(
          `Player#sendFilters() The band should be between 0 and 14.`
        );
      if (options.equalizer.gain > 1 || options.equalizer.gain < -0.25)
        throw new RangeError(
          `Player#sendFilters() The gain should be between -0.25 and 1.`
        );
      this.bands.set(options.equalizer.bands, { band: options.equalizer.bands, gain: options.equalizer.gain })
    }
    this.node
      .wsSend({
        op: "filters",
        guildId: this.options.guild.id,
        equalizer: options.equalizer ? [...this.bands.values()] : null,
        karaoke: options.karaoke ? options.karaoke : null,
        timescale: options.timescale ? options.timescale : null,
        tremolo: options.tremolo ? options.tremolo : null,
        vibrato: options.vibrato ? options.vibrato : null,
      })
      .catch((err) => {
